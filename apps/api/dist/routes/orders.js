...
ordersRouter.patch('/:orderId', async (req, res) => {
    try {
        ...
        if (parsed.data.payment_status)
            order.payment_status = parsed.data.payment_status;
        if (parsed.data.payment_id)
            order.payment_id = parsed.data.payment_id;
        if (parsed.data.instructions !== undefined)
            order.instructions = parsed.data.instructions;
 ...
 */
ordersRouter.post('/:orderId/pay', async (req, res) => {
    try {
        const user = await resolveUser(req);
        if (!user) {
            return res.status(401).json({ error: 'Authentication required' });
 ...
 */
ordersRouter.post('/:orderId/confirm-payment', async (req, res) => {
    try {
        const { orderId } = req.params;
 ...
 */
ordersRouter.get('/admin/all', async (req, res) => {
    try {
        const user = await resolveUser(req);
        if (!user) {
            return res.status(401).json({ error: 'Authentication required' });
 ...
 */
ordersRouter.patch('/admin/:orderId/status', async (req, res) => {
    try {
        const user = await resolveUser(req);
        if (!user) {
            return res.status(401).json({ error: 'Authentication required' });
