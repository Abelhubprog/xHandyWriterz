...
export const uploadCompatRouter = Router();
// R2/S3 Client
...
async function notifyAdminNewUpload(data) {
...
async function sendAdminEmail(adminEmail, data, resendKey, sendgridKey) {
...
async function resolveUser(req, allowAnonymous) {
...
async function buildPresignedPut(key, contentType, userId, contentLength) {
...
async function resolveUserFromReq(req, allowAnon) {
    const headers = {};
...
async function handlePresignGet(req, res) {
    try {
...
uploadRouter.post('/', async (req, res) => {
    try {
        ...
        const uploadId = typeof metadataRecord.submissionId === 'string' ? metadataRecord.submissionId :
...
uploadRouter.post('/notify', async (req, res) => {
    try {
        ...
        const mmWebhookUrl = process.env.MATTERMOST_WEBHOOK_URL;
        if (mmWebhookUrl) {
            const fileList = files?.map((f) => `- ${f.filename} (${formatBytes(f.size || 0)})`).join('\n') || 'No files listed';
            try {
                await fetch(mmWebhookUrl, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
        ...
        if (sendReceipt && email) {
            ...
            if (resendKey || sendgridKey) {
                const fileList = files?.map((f) => `<li>${f.filename} (${formatBytes(f.size || 0)})</li>`).join('') || '';
                const htmlContent = `
