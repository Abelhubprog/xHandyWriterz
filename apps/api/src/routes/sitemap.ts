...
async function generateSitemap(): Promise<string> {
  ...
  try {
    const servicesResponse = await fetch(
      `${CMS_URL}/api/services?populate=*&pagination[pageSize]=100&sort=updatedAt:desc`
    );

    if (servicesResponse.ok) {
      const servicesData = await servicesResponse.json() as { data?: unknown[] };
  ...
  try {
    const articlesResponse = await fetch(
      `${CMS_URL}/api/articles?populate=*&pagination[pageSize]=100&sort=updatedAt:desc`
    );

    if (articlesResponse.ok) {
      const articlesData = await articlesResponse.json() as { data?: unknown[] };
