 ...
 */
paymentsRouter.get('/status/:sessionId', async (req: Request, res: Response) => {
  try {
 ...
 */
paymentsRouter.get('/providers', (_req: Request, res: Response) => {
  interface Provider {
    id: string;
    name: string;
    icon: string;
  ...
  if (providers.length === 0) {
    providers.push(
      { id: 'stripe', name: 'Credit/Debit Card', icon: 'credit-card', currencies: ['GBP', 'EUR', 'USD'] },
      { id: 'paypal', name: 'PayPal', icon: 'paypal', currencies: ['GBP', 'EUR', 'USD'] },
      { id: 'stablelink', name: 'Crypto (StableLink)', icon: 'bitcoin', currencies: ['GBP', 'EUR'] },
      { id: 'coinbase', name: 'Coinbase', icon: 'bitcoin', currencies: ['BTC', 'ETH', 'USDC'] }
    );
  }
 ...
 */
async function notifyPaymentComplete(orderId: string | undefined, paymentId: string, provider: string): Promise<void> {
  if (!orderId) return;
