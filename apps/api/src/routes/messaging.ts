...
async function findOrCreateMMUser(clerkUser: ClerkUser, adminToken: string): Promise<MMUser | null> {
  ...
  try {
    ...
    const searchResponse = await fetch(
      `${mmUrl}/api/v4/users/email/${encodeURIComponent(email)}`,
      { headers: { Authorization: `Bearer ${adminToken}` } }
    );

    if (searchResponse.ok) {
      return await searchResponse.json() as MMUser;
    }
    ...
    if (createResponse.ok) {
      return await createResponse.json() as MMUser;
    }
...
async function createMMSession(userId: string, adminToken: string): Promise<MMSession | null> {
  const mmUrl = MATTERMOST_URL;
